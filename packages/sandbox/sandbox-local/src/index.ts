 ...
 * @module @williamcodebox/omd-sandbox-local
 */
...
} from '@williamcodebox/node-addon-landlock-run'
...
import { SandboxProvider, SandboxUnavailableError } from '@williamcodebox/omd-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@williamcodebox/omd-sandbox'
import type { SessionId } from '@williamcodebox/omd-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@williamcodebox/omd-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@williamcodebox/omd-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@williamcodebox/omd-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@williamcodebox/omd-sandbox-windows-acl/src/runner.ts'))
