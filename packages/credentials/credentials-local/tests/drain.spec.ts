...
vi.mock('@williamcodebox/omd-atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@williamcodebox/omd-atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@williamcodebox/omd-atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
