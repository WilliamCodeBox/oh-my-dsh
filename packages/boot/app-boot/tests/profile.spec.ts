...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@williamcodebox/omd-base'])
    ...
    expect(readProfileManifest('t', dir).dsh?.profile?.bundles).toEqual(['@williamcodebox/omd-base'])
...
describe('loadProfile', () => {
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @williamcodebox/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('@williamcodebox/omd-base')
    try {
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@williamcodebox/omd-base': { patch: '[]\n' },
      '@williamcodebox/omd-web-app': { patch: '[]\n' },
      '@williamcodebox/omd-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@williamcodebox/omd-base', '@williamcodebox/omd-web-app', '@williamcodebox/omd-headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).dsh?.profile?.bundles)
      .toEqual(['@williamcodebox/omd-base', '@williamcodebox/omd-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@williamcodebox/omd-base', '@williamcodebox/omd-web-app', '@williamcodebox/omd-headless', 'custom-bundle',
    ])
