...
- `ctx.cmdlineArgs`：本次调用的内层参数。`get()` 就是它的全部接口，返回一份快照：`omd --profile tui --resume abc` 得到 `['--resume', 'abc']`。
...
```yaml
- id: web-startup
  name: '@williamcodebox/omd-web-app/startup'
```
