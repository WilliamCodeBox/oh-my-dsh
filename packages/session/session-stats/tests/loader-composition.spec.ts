...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@williamcodebox/omd-session-projection', SessionProjectionRegistry],
    ['@williamcodebox/omd-session-stats', SessionStatsPlugin],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped session-stats YAML shape and serves whole-log counts', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@williamcodebox/omd-session-projection'",
      "- name: '@williamcodebox/omd-session-stats'",
    ])
