...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@williamcodebox/omd-llm'
import type { GenerateOptions, StreamChunk } from '@williamcodebox/omd-llm'
...
import * as providerPlugin from '@williamcodebox/omd-session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    "- name: '@williamcodebox/omd-llm'",
    "- name: '@williamcodebox/omd-session'",
    "- name: '@williamcodebox/omd-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@williamcodebox/omd-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ['@williamcodebox/omd-llm', LlmRuntime],
    ['@williamcodebox/omd-session', SessionStore],
    ['@williamcodebox/omd-session-title', SessionTitleService],
    ['@williamcodebox/omd-session-title-first-prompt-llm', providerPlugin],
  ])
