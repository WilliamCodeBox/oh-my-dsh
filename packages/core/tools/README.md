...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@williamcodebox/omd-tool-call-timeout-policy` wrapper.
