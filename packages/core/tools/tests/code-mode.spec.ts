...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@williamcodebox/omd-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@williamcodebox/omd-tools'
import type { Agent } from '@williamcodebox/omd-agent'
import { Session, SessionId } from '@williamcodebox/omd-session'
import type { JsonValue, SessionEventMap } from '@williamcodebox/omd-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@williamcodebox/omd-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
