 ...
 * execution pipeline.
 * @module @williamcodebox/omd-tools
 */

import { Context, Service } from '@williamcodebox/cordis'
import z from '@williamcodebox/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@williamcodebox/omd-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@williamcodebox/omd-scope'
import type { CallId, ContentBlock, ToolSchema } from '@williamcodebox/omd-llm'
import { assertNever, deepFreeze, HarnessError } from '@williamcodebox/omd-llm'
import type { Agent } from '@williamcodebox/omd-agent'
import { snapshotJsonValue } from '@williamcodebox/omd-session'
import type { JsonValue, UserMessage } from '@williamcodebox/omd-session'
import type { ToolProviderResult } from '@williamcodebox/omd-system-prompt'
import type { CodeRuntime } from '@williamcodebox/omd-code-runtime'
...
export type { JsonValue } from '@williamcodebox/omd-session'
...
// lives in its own UI-facing module; re-export it so `@williamcodebox/omd-tools`
...
declare module '@williamcodebox/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@williamcodebox/omd-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@williamcodebox/omd-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@williamcodebox/omd-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@williamcodebox/omd-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@williamcodebox/omd-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @williamcodebox/omd-code-runtime-worker-thread) or set tools mode to "native"`)
    }
