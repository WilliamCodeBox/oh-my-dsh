...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@williamcodebox/omd-llm/message'
import { CallId } from '@williamcodebox/omd-llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@williamcodebox/omd-attachment'
import type {
  SessionEvent,
  SessionId,
  TodoItem,
} from '@williamcodebox/omd-session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@williamcodebox/omd-commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@williamcodebox/omd-commands/types'
import { deriveEventMessage, foldSurface } from '@williamcodebox/omd-session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@williamcodebox/omd-host-apiproxy/api'
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@williamcodebox/omd-tool-bash'\n" }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@williamcodebox/omd-tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@williamcodebox/omd-tool-read'\n" }],
  ])
