...
const GENERATED_REMOTE = /^@williamcodebox\/omd-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
 ...
 */
const RUNTIME_STORE_EXEMPTION = '@williamcodebox/omd-client-runtime/client'
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    plugins: [{
      ...
      // and every other @williamcodebox value import is a build error — a
      ...
      resolveId(source: string) {
        if (!source.startsWith('@williamcodebox/')) return null
