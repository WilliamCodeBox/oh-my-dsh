...
afterEach(cleanup)
import { AppRoot } from '@williamcodebox/omd-client-web/src/AppRoot.tsx'
import { createLoaderStatusStore, createSignal } from '@williamcodebox/omd-client-web/src/loader-status.ts'
...
describe('AppRoot', () => {
  ...
  it('lists failed entries and stays on the loading page', () => {
    ...
    act(() => {
      status.set('@williamcodebox/omd-client-ui-layout', 'failed')
      status.set('ok', 'active')
    })
    expect(getByText('Failed to load plugins')).toBeTruthy()
    expect(getByText('@williamcodebox/omd-client-ui-layout')).toBeTruthy()
    expect(queryByTestId('real-ui')).toBeNull()
  })
