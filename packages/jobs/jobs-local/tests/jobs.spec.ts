...
import { bindScopeParent, createScope, scopeOf } from '@williamcodebox/omd-scope'
import type { ScopeKey } from '@williamcodebox/omd-scope'
import { JobId } from '@williamcodebox/omd-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@williamcodebox/omd-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @williamcodebox/omd-tool-jobs in its composition)')
  })
