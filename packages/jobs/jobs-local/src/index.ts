 ...
 * @module @williamcodebox/omd-jobs-local
 */
...
import { AnonymousEntries, ScopedLayers, scopeOf } from '@williamcodebox/omd-scope'
import type { ScopeLayer } from '@williamcodebox/omd-scope'
import { deadline, timeoutOf } from '@williamcodebox/omd-timeout'
import { JobRegistry, JobId } from '@williamcodebox/omd-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@williamcodebox/omd-jobs'
 ...
 * `@williamcodebox/omd-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @williamcodebox/omd-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @williamcodebox/omd-agent)')
    }
