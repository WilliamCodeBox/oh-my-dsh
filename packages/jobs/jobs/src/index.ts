 ...
 * process-local registry lives in `@williamcodebox/omd-jobs-local`.
 * @module @williamcodebox/omd-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@williamcodebox/omd-jobs is the abstract job registry seam; load an implementation such as @williamcodebox/omd-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
