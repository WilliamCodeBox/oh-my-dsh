 ...
 * `@williamcodebox/omd-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@williamcodebox/omd-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@williamcodebox/omd-tool-fs`),
 * exactly as bash's does.
 *
 * @module @williamcodebox/omd-fs-sandbox
 */

import { Context } from '@williamcodebox/cordis'
import { LocalFileSystem } from '@williamcodebox/omd-fs-local'
import type { Config as LocalConfig } from '@williamcodebox/omd-fs-local'
import { FsError } from '@williamcodebox/omd-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@williamcodebox/omd-fs'
import { writableRoots } from '@williamcodebox/omd-sandbox'
