 ...
 *
 * @module @williamcodebox/omd-tool-fs-search/search-core
 */

import { isAbsolute, relative, sep } from 'node:path'
import type { Context } from '@williamcodebox/cordis'
import { HarnessError } from '@williamcodebox/omd-llm'
import { ItemRetainer, TextRetainer } from '@williamcodebox/omd-output-retention'
import type { RetainedItems } from '@williamcodebox/omd-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@williamcodebox/omd-subprocess'
 ...
 * `@williamcodebox/omd-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@williamcodebox/omd-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@williamcodebox/omd-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@williamcodebox/omd-tool-fs-search/presentation} `grepSearchMeta`)
