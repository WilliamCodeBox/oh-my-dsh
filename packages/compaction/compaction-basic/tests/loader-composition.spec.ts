...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@williamcodebox/omd-token-meter', TokenMeter],
    ['@williamcodebox/omd-compaction-tool-result-pruner', ToolResultPruner],
    ['@williamcodebox/omd-compaction-basic', BasicCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-basic YAML order', async () => {
    const loaded = await loadYaml([
      ...
      '    headChars: 20',
      '    tailChars: 10',
      "- name: '@williamcodebox/omd-compaction-basic'",
      '  config:',
      '    thresholdRatio: 0.5',
      '    retainRatio: 0.125',
      '    auto: false',
