...
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@williamcodebox/omd-agent'
import type {} from '@williamcodebox/omd-agent-presets/types'
import { AttachmentError } from '@williamcodebox/omd-attachment'
import type { ImageAttachmentRef } from '@williamcodebox/omd-attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@williamcodebox/omd-llm'
import { errorChain } from '@williamcodebox/omd-llm'
import type { ContentBlock, MessageSource } from '@williamcodebox/omd-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@williamcodebox/omd-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@williamcodebox/omd-session'
import type { SessionPersistence } from '@williamcodebox/omd-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@williamcodebox/omd-session-query'
import { SubagentError } from '@williamcodebox/omd-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@williamcodebox/omd-subagent'
import { isUserInvocable } from '@williamcodebox/omd-skill'
import type { Workspace, WorkspaceRecord } from '@williamcodebox/omd-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@williamcodebox/omd-workspace'
...
} from '@williamcodebox/omd-agent-presets'
import type { PresetBearingSession } from '@williamcodebox/omd-agent-presets'
...
import type { SessionRawArtifact } from '@williamcodebox/omd-session-persistence'
import {
...
import type {} from '@williamcodebox/omd-session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@williamcodebox/omd-jobs'
import type { JobSnapshot } from '@williamcodebox/omd-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@williamcodebox/omd-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@williamcodebox/omd-goal'
import type { GoalRef as CoreGoalRef } from '@williamcodebox/omd-goal'
...
import type {} from '@williamcodebox/omd-cordis-host-runner/types'
import type {} from '@williamcodebox/omd-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@williamcodebox/omd-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@williamcodebox/omd-settings'
import { credentialRef } from '@williamcodebox/omd-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@williamcodebox/omd-session-title'
import type { CallId } from '@williamcodebox/omd-llm/brand'
import type { ScopeKey } from '@williamcodebox/omd-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@williamcodebox/omd-user-approval'
...
} from '@williamcodebox/omd-user-questions'
import { UserQuestionError } from '@williamcodebox/omd-user-questions'
import { DirectoryPickerError } from '@williamcodebox/omd-host-directory-picker'
import {
...
} from '@williamcodebox/omd-api-remotes'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @williamcodebox/omd-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @williamcodebox/omd-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @williamcodebox/omd-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @williamcodebox/omd-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @williamcodebox/omd-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @williamcodebox/omd-skill', details: {} })
        }
