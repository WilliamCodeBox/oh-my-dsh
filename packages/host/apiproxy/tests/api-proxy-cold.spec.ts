...
import { TypertLookupFailure } from '@williamcodebox/omd-typert-protocol'
import TypertRegistry from '@williamcodebox/omd-typert-registry'
import { createUserMessage, MessageId } from '@williamcodebox/omd-llm'
...
import {
  ...
  type StoredPrefix,
} from '@williamcodebox/omd-session-persistence'
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@williamcodebox/omd-session').Session
