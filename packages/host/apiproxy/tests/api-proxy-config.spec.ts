...
import LlmRuntime, { LlmAdapter } from '@williamcodebox/omd-llm'
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@williamcodebox/omd-llm'
import { SettingsProvider, settingsNamespace } from '@williamcodebox/omd-settings'
import type { SettingsNamespace } from '@williamcodebox/omd-settings'
import { CredentialProvider } from '@williamcodebox/omd-credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@williamcodebox/omd-credentials'
import type { HostFrame } from '../src/api/index.ts'
import type { RpcRequest, RpcResponse } from '../src/api/rpc.ts'
import { RpcId } from '../src/api/rpc.ts'
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@williamcodebox/omd-agent-default-model'
...
describe('settings domain', () => {
  it('reports an actionable error when no settings provider is mounted', async () => {
    ...
    expect(error.message).toContain('omd-settings-file')
  })
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('omd-credentials-local')
  })
