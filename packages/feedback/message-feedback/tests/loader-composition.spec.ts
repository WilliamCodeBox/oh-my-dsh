...
import * as StorageDomain from '@williamcodebox/omd-storage-domain'
import * as StorageJson from '@williamcodebox/omd-storage-json'
import { remoteMethods } from '@williamcodebox/omd-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@williamcodebox/omd-session-persistence-jsonl', JsonlSessionPersistence],
    ['@williamcodebox/omd-storage', Storage],
    ['@williamcodebox/omd-storage-json', StorageJson],
    ['@williamcodebox/omd-storage-domain', StorageDomain],
    ['@williamcodebox/omd-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@williamcodebox/omd-session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@williamcodebox/omd-storage'",
      "- name: '@williamcodebox/omd-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@williamcodebox/omd-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@williamcodebox/omd-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
