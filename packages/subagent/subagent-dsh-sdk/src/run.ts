 ...
 * runtime over stdio JSON-RPC through `@williamcodebox/omd-sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`@williamcodebox/omd-subagent-acp`): publish after the child handshake,
 ...
 *
 * @module @williamcodebox/omd-subagent-dsh-sdk/run
 */

import { randomUUID } from 'node:crypto'
import { DeepSeekHarness, type HarnessNotification } from '@williamcodebox/omd-sdk-client'
import type { ContentBlock } from '@williamcodebox/omd-llm'
import { SessionId, type SessionEvent, type TurnEndReason } from '@williamcodebox/omd-session'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@williamcodebox/omd-subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@williamcodebox/omd-subagent'
