 ...
 *
 * @module @williamcodebox/omd-subagent
 */

import type { Context } from '@williamcodebox/cordis'
import type { Session, SessionEvent, SessionHeader, SessionId } from '@williamcodebox/omd-session'
import type { SessionPersistence } from '@williamcodebox/omd-session-persistence'
import type { SessionProjectionRegistry } from '@williamcodebox/omd-session-projection'
import type { SessionProjectionCache } from '@williamcodebox/omd-session-projection-cache'
...
async function prepareListing(
  ...
  if (projections === undefined) {
    throw new SubagentError(
      'listing subagents requires the sessionProjections registry (load @williamcodebox/omd-session-projection)',
  ...
  if (sessions === undefined) {
    throw new SubagentError(
      'listing subagents requires the session store (load @williamcodebox/omd-session)',
