...
/**
 * Keyless built-artifact smoke: plain Node imports `@williamcodebox/omd-lsp` and
 * `@williamcodebox/omd-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@williamcodebox/cordis')
      const { default: Lsp } = await import('@williamcodebox/omd-lsp')
      const LspLocal = await import('@williamcodebox/omd-lsp-stdio')
      const { default: LocalFileSystem } = await import('@williamcodebox/omd-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@williamcodebox/omd-subprocess-local')
