...
| Package | Role |
|---|---|
| `@williamcodebox/omd-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@williamcodebox/omd-web-search-exa` | Search provider: Exa |
| `@williamcodebox/omd-web-search-perplexity` | Search provider: Perplexity |
| `@williamcodebox/omd-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@williamcodebox/omd-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
