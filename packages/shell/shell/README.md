...
| Package | Role |
|---|---|
| `@williamcodebox/omd-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@williamcodebox/omd-bash-local` | Service Provider: local subprocesses |
| `@williamcodebox/omd-bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@williamcodebox/omd-tool-bash` | the model-facing tool schemas over `ctx.shell` |
