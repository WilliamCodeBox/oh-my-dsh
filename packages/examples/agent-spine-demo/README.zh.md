...
```
@williamcodebox/cordis-plugin-timer  timer service (writes nothing to stdout)
@williamcodebox/omd-llm              abstract LLM service + content-block vocabulary
@williamcodebox/omd-session          event-sourced session log + store
@williamcodebox/omd-session-title    log-backed title service + deterministic fallback
@williamcodebox/omd-system-prompt    prompt-section + tool-schema assembly
@williamcodebox/omd-tools            registry + guarded pre/around/post/final-result pipeline
@williamcodebox/omd-skill            skill provider registry
@williamcodebox/omd-skill-filesystem      local filesystem skill provider
@williamcodebox/omd-agent            agent registry + initiator scope + agent/* events
@williamcodebox/omd-goal             optional persisted same-session goal domain
@williamcodebox/omd-tool-goal        optional model-facing goal controls
@williamcodebox/omd-goal-round-driver     optional same-session goal-round driver
@williamcodebox/omd-llm-retry        provider-routed request retry policy
@williamcodebox/omd-jobs-local      generic background-job registry
@williamcodebox/omd-invariants       configurable invariant registry service
@williamcodebox/omd-session/invariant
@williamcodebox/omd-agent/invariant
@williamcodebox/omd-scope/invariant
@williamcodebox/omd-agent-loop/invariant
                                  package-owned relational checks
@williamcodebox/omd-tool-bash        the model-facing bash schema (unless toolBash=false)
@williamcodebox/omd-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@williamcodebox/omd-tool-skill       session-prefix skill catalog + model-facing loader schema
@williamcodebox/omd-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@williamcodebox/omd-agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@williamcodebox/omd-agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@williamcodebox/omd-'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.md)。
