...
`dsh --profile tui` 表面的折叠式终端转录模型与呈现 seam。转录模型将一个会话的持久 `session/event` 流投影为展示条目；基于 pi-tui 的呈现层与交互适配器将在后续里程碑中建立在该模型之上。
...
- **仅折叠模型**——尚无视口、回滚或呈现层。渲染器里程碑将在 `Transcript` 之上构建 pi-tui 呈现层，并迁出 bundle 的逐行 tracer。
