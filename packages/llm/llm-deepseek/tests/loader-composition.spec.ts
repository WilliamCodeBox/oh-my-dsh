...
import LocalCredentialProvider from '@williamcodebox/omd-credentials-local'
import { settingsNamespace } from '@williamcodebox/omd-settings'
import FileSettingsProvider from '@williamcodebox/omd-settings-file'
...
async function loadComposition(
  ...
  await writeFile(configPath, [
    ...
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@williamcodebox/omd-settings-file'",
        '  config:',
        `    path: ${JSON.stringify(settingsPath)}`,
        '    debounceMs: 10',
        '- id: credentials',
        "  name: '@williamcodebox/omd-credentials-local'",
        '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@williamcodebox/omd-settings-file', FileSettingsProvider],
    ['@williamcodebox/omd-credentials-local', LocalCredentialProvider],
    ['@williamcodebox/omd-llm-deepseek', LlmDeepSeek],
  ])
  ctx.loader.internal = {
    version: 'v2',
