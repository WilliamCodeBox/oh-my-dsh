...
import AgentRegistry, { agentEvents, Inbox } from '@williamcodebox/omd-agent'
import type { Agent } from '@williamcodebox/omd-agent'
import { createUserMessage, HarnessError } from '@williamcodebox/omd-llm'
import SessionStore, { Session, SessionId, type UserMessage } from '@williamcodebox/omd-session'
import GoalService, {
  GoalError,
  GoalId,
  decodeGoalChange,
  foldGoal,
} from '@williamcodebox/omd-goal'
import type { GoalChangeMeta, GoalRef, GoalSnapshotChangeMeta } from '@williamcodebox/omd-goal'
...
function stubAgent(rawId: string, seed?: readonly import('@williamcodebox/omd-session').SessionEvent[]): StubAgent {
