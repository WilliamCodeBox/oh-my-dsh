...
import * as GoalSessionInvariant from '@williamcodebox/omd-goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@williamcodebox/omd-goal-round-driver'
import InvariantRegistry, { InvariantError } from '@williamcodebox/omd-invariants'
import SessionStore, { SessionId, type Session } from '@williamcodebox/omd-session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@williamcodebox/omd-goal-round-driver',
    }))
  })
