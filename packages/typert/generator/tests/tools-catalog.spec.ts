...
import type { TypertContribution } from '@williamcodebox/omd-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@williamcodebox/omd-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@williamcodebox/omd-tools'],
    }).analyze()
    ...
    const artifact = new FaceModelEmitter(host).emit('@williamcodebox/omd-tools')
    ...
    const record = ctx.typert.getPackage('@williamcodebox/omd-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@williamcodebox/omd-tools', 'host')).toBeUndefined()
  })
})
