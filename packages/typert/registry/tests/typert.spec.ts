...
} from '@williamcodebox/omd-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@williamcodebox/omd-typert-protocol'
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@williamcodebox/omd-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@williamcodebox/omd-tools', 'ToolInput')).toBe('@williamcodebox/omd-tools#ToolInput')
    expect(typertPackageKey('@williamcodebox/omd-tools', 'host')).toBe('@williamcodebox/omd-tools#host')
    expect(ctx.typert.get('@williamcodebox/omd-tools#ToolInput')).toMatchObject({
      package: '@williamcodebox/omd-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@williamcodebox/omd-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@williamcodebox/omd-tools', 'host')).toMatchObject({
      key: '@williamcodebox/omd-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@williamcodebox/omd-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@williamcodebox/omd-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@williamcodebox/omd-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@williamcodebox/omd-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@williamcodebox/omd-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@williamcodebox/omd-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@williamcodebox/omd-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@williamcodebox/omd-tools#ToolInput')).not.toBe(projected)
  })
