/**
 * @williamcodebox/omd-web-app — the browser-surface bundle's runtime glue plugin
 ...
 * @module @williamcodebox/omd-web-app
 */
...
import { addHarnessSourceSection } from '@williamcodebox/omd-app-boot'
import * as FrontendStatic from '@williamcodebox/omd-host-frontend-static'
...
/** Model-visible orientation and acceptance boundary for sessions created through `omd web`. */
function webSurfacePrompt(webUrl: string): string {
  ...
  return `You are interacting with the user through the DeepSeek Harness Web GUI at ${webUrl}. `
    ...
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only omd web injects window.__DSH_BOOT__. '
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('@williamcodebox/omd-web-frontend/dist/index.html')
  } catch {
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (config.printUrl) {
    ...
    const printUrl = (): void => {
      ...
      console.log(`omd web: ${localWebUrl(ctx)}${lanCandidate === undefined ? '' : ` (LAN: http://${lanCandidate}:${String(port)})`}`)
    }
