...
`SDK` means the JSON-RPC-based client/server protocol used by the supported Python and TypeScript SDKs. The repository keeps `@williamcodebox/omd-sdk-client`, `@williamcodebox/omd-sdk-protocol`, and the wire identity `deepseek-harness-sdk-runtime`; the JSON-RPC server belongs to the same family. DeepSeek Harness itself is not an SDK, and the removed project generator, launcher, helper, and launcher telemetry packages stay absent.
...
|---|---|---|
| `@williamcodebox/omd-jsonrpc` | `@williamcodebox/omd-sdk-jsonrpc-server` | It is the server half of the SDK protocol. `jsonrpc` alone names an encoding; `sdk-jsonrpc-server` gives the family, mechanism, and role. |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | The class is one JSON-RPC server implementation, not every possible SDK server. |

Keep `@williamcodebox/omd-sdk-client`, `@williamcodebox/omd-sdk-protocol`, and `deepseek-harness-sdk-runtime`. Exclude `@williamcodebox/create-sdk`, `@williamcodebox/omd-scripts`, `@williamcodebox/omd-helper`, and `@williamcodebox/omd-telemetry`; the separate removal decision deletes them and their support graph.
...
| `@williamcodebox/omd-bash`, `ctx.bash` | `@williamcodebox/omd-shell`, `ctx.shell` | PowerShell already implements this seam. The capability is shell execution, not Bash. |
...
| `@williamcodebox/omd-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@williamcodebox/omd-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | The environment registry is shared by Bash and PowerShell tools. |
...
| `@williamcodebox/omd-pty`, `ctx.pty`, `PtyService` | `@williamcodebox/omd-terminal`, `ctx.terminals`, `TerminalSessionService` | Callers manage multiple named terminal sessions. They do not allocate raw PTYs through this service. |
...
| `@williamcodebox/omd-pty-local`, `LocalPtyBackend` | `@williamcodebox/omd-terminal-bash`, `BashTerminalBackend` | The provider depends on Bash prompt and shell behavior. `local` hides the actual dialect. |
| `@williamcodebox/omd-tool-pty` | `@williamcodebox/omd-tool-terminal` | The model-facing tools are already `terminal_*`; the package should use the same product noun. |
...
| Former | Current | Reason |
|---|---|---|
| `@williamcodebox/omd-lsp-local` | `@williamcodebox/omd-lsp-stdio` | The provider speaks LSP over stdio through replaceable filesystem and subprocess services. It is not necessarily local. |
...
| `@williamcodebox/omd-tasks`, `ctx.tasks`, `TaskService` | `@williamcodebox/omd-jobs`, `ctx.jobs`, `JobRegistry` | The service registers, owns, observes, waits for, and cancels multiple background jobs. It is a registry, not a general task service. |
...
| `@williamcodebox/omd-tasks-local`, `LocalTaskService` | `@williamcodebox/omd-jobs-local`, `LocalJobRegistry` | This is the process-local provider of the job registry. Here `local` is meaningful because the jobs and callbacks live in one process. |
| `@williamcodebox/omd-tool-tasks` | `@williamcodebox/omd-tool-jobs` | The consumer controls the job registry and should use the same domain noun. |
...
| `@williamcodebox/omd-client-ui-task`, `client/ui-task/` | `@williamcodebox/omd-client-ui-jobs`, `client/ui-jobs/` | The client package presents the background-job collection. It is not one user task. |
...
| Former | Current | Reason |
|---|---|---|
| `@williamcodebox/omd-client-ui-slash`, `ui-slash/` | `@williamcodebox/omd-client-ui-input-trigger`, `ui-input-trigger/` | The client handles `/`, `@`, keyboard arbitration, candidate menus, and programmatic launch. It is not only slash commands. |
...
| `@williamcodebox/omd-agent-tool-mode`, plugin `tool-mode` | `@williamcodebox/omd-agent-tool-presentation`, plugin `tool-presentation` | The plugin changes how tools are presented to the model. It does not change execution behavior. Keep local `Config.mode` and `ToolPresentationMode`. |
...
| `@williamcodebox/omd-permission`, `ctx.permission`, `PermissionService` | `@williamcodebox/omd-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | The service selects and persists presets. Sandbox and approval services enforce the result. |
| `@williamcodebox/omd-client-ui-permission` | `@williamcodebox/omd-client-ui-permission-presets` | The UI edits and selects permission presets. |
...
| `@williamcodebox/omd-user-interaction`, `user-interaction/` | `@williamcodebox/omd-user-questions`, `user-questions/` | The seam supports question batches and answers only. Approval, commands, and directory picking are separate interaction seams. |
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | These names state the one supported interaction form. Keep `AskUserQuestion*`, the `ask_user_question` tool, and `@williamcodebox/omd-tool-ask-user`. |
...
| Former | Current | Reason |
|---|---|---|
| `packages/typert/type-meta/`, `@williamcodebox/omd-type-meta` | `typert/protocol/`, `@williamcodebox/omd-typert-protocol` | The package owns the Typert Remote protocol, decorators, bindings, codecs, lookups, and context contracts. It is not generic type metadata. |
...
Keep `@williamcodebox/omd-tools` and `ctx.tools`. Keep `@williamcodebox/omd-api-gateway`, its `gateway/` folder, `ctx.typertGateway`, and `TypertGatewayService`; that service is a real API gateway. Its internal `TypeRT*` identifiers still follow the `Typert*` spelling rule.

### Workspace instructions, telemetry, identity, and launch environment

| Former | Current | Reason |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` owns multiple workspaces, but Client `ctx.workspaces` already has an incompatible type. Both declarations merge into the same Cordis `Context` interface at compile time even though their runtime contexts are separate. The role suffix states the host service and avoids that collision. Keep `@williamcodebox/omd-workspace`, `WorkspaceRegistry`, `Workspace`, and `workspace.*` wire names. |
| `@williamcodebox/omd-workspace-context`, `context/workspace-context/` | `@williamcodebox/omd-agent-instructions`, `context/agent-instructions/` | The package loads hierarchical `AGENTS.md` and `CLAUDE.md` files for the agent. It is not general workspace context. |
...
| `session/user-id/`, `@williamcodebox/omd-user-id` | `identity/anonymous-user-id/`, `@williamcodebox/omd-anonymous-user-id` | The value is a random correlation id shared by telemetry, feedback, and DeepSeek requests. It is neither a Session concern nor an authenticated user identity. |
...
| `util/environment/`, `@williamcodebox/omd-environment` | `util/launch-environment/`, `@williamcodebox/omd-launch-environment` | The package captures one immutable layered snapshot at launch. It is not a general environment API. |
...
| Former | Current | Reason |
|---|---|---|
| `@williamcodebox/omd-tool-schedule`, `schedule/tool-schedule/`, plugin `tool-schedule` | `@williamcodebox/omd-schedule`, `schedule/schedule/`, plugin `schedule` | The package owns the durable Schedule domain, persistence barriers, management tools, timers, follow-ups, and runtime lifecycle. `tool-` describes only one part. |
...
| `WorkflowService`, `ctx.workflows` | `WorkflowEngine`, `ctx.workflowEngine` | One engine parses and executes workflow programs. The plural key wrongly suggests a registry. Keep `@williamcodebox/omd-workflow` and workflow events and tools. |
| `@williamcodebox/omd-workflow-workerthread`, `WorkerWorkflowEngine` | `@williamcodebox/omd-workflow-worker-thread`, `WorkerThreadWorkflowEngine` | `worker thread` is the precise Node mechanism and the repository spelling uses the full words. |
| `@williamcodebox/omd-goal-session`, `goal/goal-session/` | `@williamcodebox/omd-goal-round-driver`, `goal/goal-round-driver/` | The plugin drives same-session Goal Rounds. It neither stores goals nor defines sessions. Keep `GoalService`, goal source, events, and contracts. |
| `packages/compact/` | `packages/compaction/` | The group is a noun-domain family. `compact` remains the user command verb. |
| `@williamcodebox/omd-compact`, `ctx.compact`, `CompactService` | `@williamcodebox/omd-compaction`, `ctx.compaction`, `CompactionEngine` | The object runs the compaction algorithm and lifecycle. It is an engine, not a generic service. |
...
| `@williamcodebox/omd-compact-basic`, `BasicCompactService`, public `BasicCompact*` | `@williamcodebox/omd-compaction-basic`, `BasicCompactionEngine`, corresponding `BasicCompaction*` | `basic` is plain but honest. `compaction-llm` adds no information because LLM use is already part of the current implementation family. |
| `@williamcodebox/omd-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@williamcodebox/omd-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | The plugin is an actor that prunes tool results. The noun `pruner` names that role. |
...
| `@williamcodebox/omd-settings-local`, `SettingsLocal` | `@williamcodebox/omd-settings-file`, `FileSettingsProvider` | The implementation is file-backed through the filesystem seam. `file` states the mechanism; `local` does not. |
...
| Former | Current | Reason |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | The server owns HTTP routes and WebSocket upgrade routes. `Web` leaves room for both; `Http` is too narrow here. Keep `packages/host/webserver`, `@williamcodebox/omd-host-webserver`, `WebRoute`, and `WebUpgradeRoute`. |
...
| `@williamcodebox/omd-session-title-first-message-llm`, cadence `first-message` | `@williamcodebox/omd-session-title-first-prompt-llm`, cadence `first-prompt` | The trigger is the first user prompt, not any message in the session log. |
| `@williamcodebox/omd-session-title-all-messages-llm`, cadence `all-user-messages` | `@williamcodebox/omd-session-title-all-prompts-llm`, cadence `all-prompts` | The backend refreshes from user prompts. `all messages` wrongly includes assistant and tool events. |
| `@williamcodebox/omd-code-runtime-worker`, `WorkerCodeRuntime` | `@williamcodebox/omd-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | The implementation uses a Node worker thread. `worker` alone is too broad. |
...
| Former | Current | Reason |
|---|---|---|
| `@williamcodebox/omd-fs-policy` | `@williamcodebox/omd-fs-observation-policy` | The package defines which filesystem observations authorize later effects. It is not the complete filesystem or sandbox policy. |
...
| `@williamcodebox/omd-skill-local`, `LocalSkillProvider`, provider id `local` | `@williamcodebox/omd-skill-filesystem`, `FileSystemSkillProvider`, provider id `filesystem` | The provider discovers skill files through `ctx.fs`, which can be local or remote. The mechanism is filesystem access, not locality. |
...
| `@williamcodebox/omd-subagent-spawn`, `SpawnProvider` | `@williamcodebox/omd-subagent-spawn-in-process`, `SpawnInProcessProvider` | This provider starts a child agent in the current process. The configured provider id remains `spawn`. |
| `@williamcodebox/omd-subagent-fork`, `ForkProvider` | `@williamcodebox/omd-subagent-fork-in-process`, `ForkInProcessProvider` | This provider forks an agent in the current process. The configured provider id remains `fork`. |
| `@williamcodebox/omd-subagent-inprocess`, `subagent-inprocess/` | `@williamcodebox/omd-subagent-in-process-driver`, `subagent-in-process-driver/` | The package contains common in-process driving logic, not a third provider. |
...
| `@williamcodebox/omd-web-fetch-local`, `LocalFetchProvider`, `LocalFetchLimits`, provider id `local-http` | `@williamcodebox/omd-web-fetch-http`, `HttpFetchProvider`, `HttpFetchLimits`, provider id `http` | This provider performs direct HTTP fetches. `local` says where code happens to run, not which mechanism it provides. |

Keep `@williamcodebox/omd-subagent-dsh-sdk`, its provider id `dsh-sdk`, external ACP, Codex, and Claude Code provider families, the subagent tool package names, the main filesystem package and backends, filesystem tools and events, and the skill badge and tool packages.

### Hooks, guards, plan mode, extensions, and diagnostics

| Former | Current | Reason |
|---|---|---|
| `@williamcodebox/omd-hooks-claude`, `ClaudeHookConfig`, `parseClaudeConfig`, dialect `claude` | `@williamcodebox/omd-hooks-claude-code`, `ClaudeCodeHookConfig`, `parseClaudeCodeConfig`, dialect `claude-code` | The hook bridge targets Claude Code, not every Anthropic or Claude product. |
| `@williamcodebox/omd-repeat-tool-guard`, plugin/source `repeat-tool-guard` | `@williamcodebox/omd-repeat-tool-reminder`, plugin/source `repeat-tool-reminder` | The plugin adds a model reminder. It does not block or enforce a guard decision. |
| `@williamcodebox/omd-timeout-policy` | `@williamcodebox/omd-tool-call-timeout-policy` | The full `tool-call` qualifier names what the policy limits without calling the plugin a model-facing tool. Keep its `guard/timeout-policy/` directory and plugin id `timeout-policy`; the `packages/*/tool-*` catalog convention still applies only to packages that register tools. |
...
| `InvariantService` | `InvariantRegistry` | The object owns registered invariant checks. Keep `@williamcodebox/omd-invariants` and `ctx.invariants`. |
...
| Former | Current | Reason |
|---|---|---|
| `util/paths/`, `@williamcodebox/omd-paths` | `util/home-paths/`, `@williamcodebox/omd-home-paths` | The helpers resolve paths under the Harness home. They are not a general path library. Keep the individual function names when they already state the returned path. |
| `util/retention/`, `@williamcodebox/omd-retention` | `util/output-retention/`, `@williamcodebox/omd-output-retention` | The policy retains command and tool output. It is not a general data-retention framework. |
| `E2BSandboxService` | `E2BRuntime` | The class creates, reuses, and disposes the E2B execution environment used by filesystem and subprocess adapters. It is broader than one sandbox handle and narrower than a generic owner. Keep `@williamcodebox/omd-e2b`, `ctx.e2b`, and the `e2b/` group. |
| `@williamcodebox/omd-frontend-static` | `@williamcodebox/omd-host-frontend-static` | The package is the Host plugin that serves the frontend assets. The prefix distinguishes it from frontend application code. |
...
| `@williamcodebox/omd-jsonrpc-demo` | `@williamcodebox/omd-sdk-jsonrpc-demo` | The example demonstrates the runtime SDK over JSON-RPC. It belongs to the one SDK meaning. |
| `@williamcodebox/omd-frontend` | `@williamcodebox/omd-web-frontend` | The application is the web frontend. Keep its physical `apps/web/` folder. |
...
| `@williamcodebox/omd-client-ui-model` | `@williamcodebox/omd-client-ui-model-selection` | The package controls the model selection for a session. The singular `model` name is too broad. |
...
| `@williamcodebox/omd-client-ui-models` | `@williamcodebox/omd-client-ui-settings-models` | This package owns the Models settings panel. Keep `ModelsSettingsStore`; it holds one settings view model with data operations and subscriptions and is a real store. |
| `@williamcodebox/omd-client-ui-plugin-config`, `client/ui-plugin-config/` | `@williamcodebox/omd-client-ui-settings-plugins`, `client/ui-settings-plugins/` | This package owns the Plugins settings section, not a general plugin-configuration system. The target joins the `ui-settings-*` family and uses the section's plural product name. |
...
| `@williamcodebox/omd-client-ui-plugins`, `client/ui-plugins/`, Loader id `ui-plugins`, `client-ui-plugins-invariant` | `@williamcodebox/omd-client-ui-settings-plugin-inventory`, `client/ui-settings-plugin-inventory/`, Loader id `ui-settings-plugin-inventory`, `client-ui-settings-plugin-inventory-invariant` | This later package owns the read-only Plugin Inventory tab in the Plugins settings section. `ui-plugins` is too broad and does not distinguish the inventory from editable plugin settings. |
...
| `@williamcodebox/omd-client-ui-feedback`, `client/ui-feedback/`, Loader id `ui-feedback`, `client-ui-feedback-invariant` | `@williamcodebox/omd-client-ui-message-feedback`, `client/ui-message-feedback/`, Loader id `ui-message-feedback`, `client-ui-message-feedback-invariant` | This package presents ratings and notes for assistant messages through the `messageFeedback` Remote. The old name also appears to cover command feedback and any later feedback UI. It does not. |
...
| `@williamcodebox/omd-client-ui-question` | `@williamcodebox/omd-client-ui-user-questions` | The UI presents the user-question seam, not an arbitrary question domain. |
| `@williamcodebox/omd-client-ui-command`, `ui-command/` | `@williamcodebox/omd-client-ui-commands`, `ui-commands/` | The package presents and runs a collection of commands. |
| `@williamcodebox/omd-client-ui-directory-picker`, `client/ui-directory-picker/`, Loader id `ui-directory-picker`, `client-ui-directory-picker-invariant` | `@williamcodebox/omd-client-ui-directory-picker-browse`, `client/ui-directory-picker-browse/`, Loader id `ui-directory-picker-browse`, `client-ui-directory-picker-browse-invariant` | The Client packages now contain separate `browse` and `native` directory-picker presentations. The unqualified package is the browse implementation, not their shared definition. The target matches the Host backend family and changes no boundary. |
...
- Keep `@williamcodebox/omd-api-gateway`, `ctx.typertGateway`, and `TypertGatewayService`.
- Keep session projection names. A projection is not only a reducer function.
- Keep `@williamcodebox/omd-session-stats`, `sessionStats`, and `SessionStatsProjection`. They accurately name whole-session statistics and the maintained read model that carries them.
...
- Keep `@williamcodebox/omd-client-ui-directory-picker-native`; its suffix names the native-chooser presentation beside the renamed `-browse` variant. Keep `SURFACE_PACKAGES`; within the directory-picker auto selector it is the package map for the Client presentation half, contrasted with `BACKEND_PACKAGES`.
- Keep `@williamcodebox/omd-host-plugin-inventory`, `ctx.pluginInventory`, the `pluginInventory/list` Remote, and the `PluginInventory*` payload types. They accurately name the Host-owned read-only inventory; only the adapter class and the overly broad Client presentation names change.
