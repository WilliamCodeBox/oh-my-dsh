...
1. `@williamcodebox/omd-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@williamcodebox/omd-web-search-exa`, `@williamcodebox/omd-web-search-perplexity`, `@williamcodebox/omd-web-search-deepseek`, and `@williamcodebox/omd-web-fetch-http`.
3. `@williamcodebox/omd-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@williamcodebox/omd-tool-web  --depends on-->  @williamcodebox/omd-web  <--depends on--  @williamcodebox/omd-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @williamcodebox/omd-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @williamcodebox/omd-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @williamcodebox/omd-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@williamcodebox/omd-web-search-exa"] -->|registerSearchProvider| web["@williamcodebox/omd-web / ctx.web"]
  perplexity["@williamcodebox/omd-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@williamcodebox/omd-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@williamcodebox/omd-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@williamcodebox/omd-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@williamcodebox/omd-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@williamcodebox/omd-tool-web` depends on `@williamcodebox/omd-web`, `@williamcodebox/omd-tools`, `@williamcodebox/omd-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@williamcodebox/omd-web'
...
- id: web-search-exa
  name: '@williamcodebox/omd-web-search-exa'

- id: web-search-perplexity
  name: '@williamcodebox/omd-web-search-perplexity'

- id: web-search-deepseek
  name: '@williamcodebox/omd-web-search-deepseek'

- id: web-fetch-http
  name: '@williamcodebox/omd-web-fetch-http'
