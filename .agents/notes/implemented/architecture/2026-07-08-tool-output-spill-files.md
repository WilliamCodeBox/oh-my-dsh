...
| Package | Role |
|---|---|
| `@williamcodebox/omd-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@williamcodebox/omd-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@williamcodebox/omd-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- id: spill-local
  name: '@williamcodebox/omd-spill-local'

- id: spill-policy
  name: '@williamcodebox/omd-spill-policy'
  config:
...
- `@williamcodebox/omd-output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@williamcodebox/omd-spill` owns saving final text and returning a locator plus retrieval hint.
- `@williamcodebox/omd-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
