...
The old Agent Note already deferred a separate `@williamcodebox/omd-fs-observation-policy` package. This decision builds that layer and keeps `ctx.fs` close to fsspec-style storage primitives (`info`/`cat`/`open`), without turning it into full fsspec.
...
`@williamcodebox/omd-fs` shrinks to provider text IO plus guarded text mutation:
