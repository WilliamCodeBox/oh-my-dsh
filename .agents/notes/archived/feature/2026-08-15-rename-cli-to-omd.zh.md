# Agent Note: CLI 可执行名改名为 omd

Status: implemented
Archived: 2026-08-16

[English](2026-08-15-rename-cli-to-omd.md) | 中文

## 问题

官方 `@williamcodebox/dsh` CLI 已在本机全局安装（且已发布到 npm），本地 `dsh` 可执行文件会遮蔽或与之冲突。oh-my-dsh fork 的启动器需要自己的命令名。
...
- 保留的技术标识符：`@williamcodebox/omd-*` 包名、`DSH_HOME`/`DSH_*` 环境变量、`dsh.bundle`/`dsh.profile` manifest 键、`~/.dsh` home、`dsh-` 前缀的插件/特性名（如 `dsh-badge` skill）——那些是协议或包标识，不是命令名。
