...
目标集合只包含 `packages/*/*/package.json` 与 `apps/*/package.json` 中命名为 `@williamcodebox/*` 的 workspace 包。根项目、`website/`、vendor、Python 与 native workspace 不属于该 NPM 基线。发现机制必须拒绝重复包名、不同基础版本、意外的 `private` 发布状态以及集合中的未知包，而不是维护另一份手工包名列表。
...
- `@williamcodebox/oh-my-dsh` 安装后的 `omd --version` 与 `omd --dump-default-config` 在普通 Node 下成功，分别覆盖静态 CLI 入口和一个动态模式入口。
...
把所有 tarball 都安装为临时项目的顶层依赖可能掩盖未声明的内部依赖。测试生成器应按被测应用的声明式递归闭包安装，并结合现有依赖门禁；对依赖面接近全集的 `@williamcodebox/oh-my-dsh`，仍需依靠 package manifest 与静态图检查发现未声明边。
