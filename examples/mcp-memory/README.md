...
These three **default-off reference configurations** connect one memory system to DSH through [`@williamcodebox/omd-mcp-client`](../../packages/mcp/mcp-client/README.md). Pick one, or copy the same generic MCP row for another server.
...
```sh
omd web --patch "$PWD/examples/mcp-memory/memorix.cordis.yml"
```
...
omd web --patch "$PWD/examples/mcp-memory/mcp-reference-memory.cordis.yml"
```
...
omd web --patch "$PWD/examples/mcp-memory/engram.cordis.yml"
```
...
- insert:
    - id: memory-my-server
      name: '@williamcodebox/omd-mcp-client'
      config:
