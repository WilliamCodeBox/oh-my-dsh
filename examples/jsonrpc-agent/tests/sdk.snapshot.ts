 ...
 * REAL `@williamcodebox/omd-sdk-client`, drives one turn over stdio JSON-RPC,
...
} from '@williamcodebox/omd-acp-snapshot'
import { resolveExampleLaunch } from '@williamcodebox/omd-loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@williamcodebox/omd-sdk-client'
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@williamcodebox/omd-system-prompt') return []
