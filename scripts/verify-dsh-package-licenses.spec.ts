...
describe('DSH package license gate', () => {
  it('checks root, unhyphenated CLI, and omd-prefixed package names while ignoring other families', () => {
    ...
    expect(inspectDshPackageLicenses(root)).toEqual({
      packageCount: 3,
      failures: [
        'packages/core/agent/package.json: @williamcodebox/omd-agent must declare "license": "MIT"; found "BSD-3-Clause".',
      ],
    })
  })
  ...
  it('rejects a missing license declaration', () => {
    const root = createWorkspace()
    writeManifest(root, 'packages/core/agent/package.json', { name: '@williamcodebox/omd-agent' })

    expect(inspectDshPackageLicenses(root).failures).toEqual([
      'packages/core/agent/package.json: @williamcodebox/omd-agent must declare "license": "MIT"; found undefined.',
    ])
  })
})
