/**
 * Rescope the vendored Cordis packages into the `@williamcodebox` scope, and undo
 ...
 * `@williamcodebox/omd-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@williamcodebox/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@williamcodebox/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@williamcodebox/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@williamcodebox/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@williamcodebox/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@williamcodebox/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@williamcodebox/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@williamcodebox/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@williamcodebox/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@williamcodebox/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@williamcodebox/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@williamcodebox\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@williamcodebox/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@williamcodebox/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@williamcodebox/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@williamcodebox/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@williamcodebox/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@williamcodebox/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@williamcodebox` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@williamcodebox\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@williamcodebox/cordis']
    const dev = manifest.devDependencies?.['@williamcodebox/cordis']

    if (!peer) errors.push(\`\${label}: @williamcodebox/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @williamcodebox/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @williamcodebox/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
    // The rescoped name is already covered by the `@williamcodebox/.+` pattern beside it.
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@williamcodebox/\')) {',
    replace: `      // Vendored packages are rescoped too (vendor/README.md), so publication
      ...
      if (!name.startsWith('@williamcodebox/')) {`,
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@williamcodebox/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: '@williamcodebox/cordis-plugin-timer  timer service',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@williamcodebox/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@williamcodebox/` as "another plugin package".
 ...
 */
const VENDORED_LIBRARY = /^@williamcodebox\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@williamcodebox/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@williamcodebox\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@williamcodebox/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
  ...
  // rescoped into @williamcodebox, so the consumer installs this repository's
