...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/bundle/tui': { kind: 'none', reason: 'The interactive runner submits user input as ordinary user messages; prompts and tools belong to the composed base and tui bundles.' },
  'packages/interaction/tui-renderer': { kind: 'none', reason: 'The transcript model folds durable session events into display items; it registers no prompt, tool schema, or model context of its own.' },
  ...
  'packages/subagent/subagent-cli': { kind: 'indirect', reason: 'The child CLI backend delegates model rendering to dsh-tool-subagent.' },
