...
function checkManifest(
  ...
  if (owner.packageName === '@williamcodebox/omd-invariants') return
  if (manifest.peerDependencies?.['@williamcodebox/omd-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@williamcodebox/omd-invariants must be a workspace:^ peerDependency',
    )
  }
  if (manifest.devDependencies?.['@williamcodebox/omd-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@williamcodebox/omd-invariants must also be a workspace:^ devDependency',
    )
  }
}
...
function checkBuild(
  ...
  if (owner.packageName !== '@williamcodebox/omd-invariants'
