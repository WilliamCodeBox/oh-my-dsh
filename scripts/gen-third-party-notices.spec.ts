...
describe('tierExternalDeps', () => {
  it('tiers by declaring area, not by the declaring section name', () => {
    const { manifests, names } = workspace({
      ...
      'packages/test-support/loader-smoke/package.json': { name: '@williamcodebox/omd-loader-smoke', dependencies: { 'smoke-helper': '^1' } },
      'packages/test-support/client-runtime/package.json': { name: '@williamcodebox/omd-client-test-runtime', dependencies: { 'test-lib': '^1' } },
      ...
      'packages/mcp/mcp-client/package.json': { name: '@williamcodebox/omd-mcp-client', dependencies: { 'protocol-sdk': '^1' }, devDependencies: { 'protocol-fixture-server': '^1' } },
      'apps/cli/package.json': { name: '@williamcodebox/omd-cli', dependencies: { 'cli-lib': '^1', '@williamcodebox/omd-mcp-client': 'workspace:^' } },
    })
  ...
  it('keeps a package runtime when any shipping area declares it, and excludes workspace links', () => {
    const { manifests, names } = workspace({
      ...
      'packages/interaction/tui/package.json': { name: '@williamcodebox/omd-tui', dependencies: { shared: '^1', '@williamcodebox/omd-cli': 'workspace:^' } },
      'apps/cli/package.json': { name: '@williamcodebox/omd-cli' },
    })

    expect(tierExternalDeps(manifests, names).get('shared')).toBe(true)
    expect(tierExternalDeps(manifests, names).has('@williamcodebox/omd-cli')).toBe(false)
  })
})
...
describe('parseVendoredRows', () => {
  ...
  it('yields nothing when the table columns change, so the generator fails loud', () => {
    expect(parseVendoredRows('| `cordis/` | `@williamcodebox/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\n')).toEqual([])
  })
