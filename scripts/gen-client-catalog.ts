...
/** Drop the `@williamcodebox/omd-` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@williamcodebox/omd-', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @williamcodebox/omd-cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
