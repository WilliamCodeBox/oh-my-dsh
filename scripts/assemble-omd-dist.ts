...
const outPath = flag('out') ?? '/tmp/omd-dist.tar.gz'
...
try {
  ...
  // 2. trim to publish semantics: drop src/tests/docs/build metadata
  ...
  // 4. smoke: version must resolve through the assembled tree
  run(`DSH_HOME=${join(work, '.dsh-smoke')} ${join(work, 'omd')} --version`, work)

  // 5. pack
  run(`tar czf ${outPath} -C ${work} .`, work)
