...
const RELEASE_ENTRY_PACKAGE = '@williamcodebox/dsh'
...
while time.monotonic() < deadline:
    ...
    if not termination_sent and b"omd web: http://127.0.0.1:" in snapshot:
        ready_seen = True
...
if not ready_seen:
    sys.stderr.write("installed omd web did not reach its ready URL\n")
    sys.exit(124)
actual_exit = os.waitstatus_to_exitcode(status)
if actual_exit != 0:
    sys.stderr.write(f"installed omd web exited {actual_exit}, expected 0\n")
    sys.exit(125)
`
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@williamcodebox/')) {
        throw new Error(`${manifestPath} must name an @williamcodebox package`)
      }
      if (name === '@williamcodebox/omd-root') {
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@williamcodebox/dsh/lib/bin.js')
      ...
      if (version !== this.bundle.manifest.version) {
        throw new Error(
          `installed omd --version returned ${JSON.stringify(version)}; `
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@williamcodebox/') || name === '@williamcodebox/omd-root')) {
