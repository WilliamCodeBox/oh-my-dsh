...
const MERGE_HEAD = /declare module ['"](?:@williamcodebox\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@williamcodebox/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@williamcodebox/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@williamcodebox/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@williamcodebox/cordis'` Context merge
...
/**
 * Every event name a `declare module '@williamcodebox/cordis'` Events merge declares in one
