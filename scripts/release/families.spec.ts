...
describe('release families', () => {
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const dsh = releaseFamily('dsh')
    const members = [member('apps/cli', '@williamcodebox/dsh'), { ...member('apps/web', '@williamcodebox/omd-web-frontend'), version: '0.0.2' }]
  ...
  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@williamcodebox/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@williamcodebox/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@williamcodebox/omd-consumer', { dependencies: { '@williamcodebox/omd-library': 'workspace:^' } }),
      member('packages/a/library', '@williamcodebox/omd-library'),
      member('packages/a/zebra', '@williamcodebox/omd-zebra'),
    ]

    expect(dsh.publishOrder(members).map(entry => entry.name)).toEqual([
      '@williamcodebox/omd-library',
      '@williamcodebox/omd-consumer',
      '@williamcodebox/omd-zebra',
    ])
  })
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@williamcodebox/omd-left', { dependencies: { '@williamcodebox/omd-right': 'workspace:^' } }),
      member('packages/a/right', '@williamcodebox/omd-right', { dependencies: { '@williamcodebox/omd-left': 'workspace:^' } }),
    ]
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    const dsh = releaseFamily('dsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@williamcodebox/omd-library')
    const vendored = member('vendor/cordis', '@williamcodebox/cordis')
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@williamcodebox/dsh', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@williamcodebox/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@williamcodebox/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@williamcodebox/omd-library', { files: ['lib/index.js'] }),
