...
const CHOOSER_PACKAGE = '@williamcodebox/omd-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@williamcodebox/omd-host-directory-picker-native',
  '@williamcodebox/omd-host-directory-picker-browse',
  '@williamcodebox/omd-client-ui-directory-picker-browse',
  '@williamcodebox/omd-client-ui-directory-picker-native',
]
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if ((value.group === true || value.name === '@williamcodebox/cordis-plugin-group') && isUnknownArray(value.config)) {
  ...
  if (value.name !== '@williamcodebox/cordis-plugin-include') return
