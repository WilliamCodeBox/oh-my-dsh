...
function workspaceFixture(options: {
  ...
  writeJson(join(shared, 'package.json'), { name: '@williamcodebox/omd-shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@williamcodebox/omd-split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@williamcodebox/omd-host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@williamcodebox/omd-client-consumer' })
