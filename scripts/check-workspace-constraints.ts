...
const vendoredPackages = new Set([
  '@williamcodebox/cordis',
  '@williamcodebox/cosmokit',
  '@williamcodebox/schemastery',
  '@williamcodebox/cordis-plugin-loader',
  '@williamcodebox/cordis-plugin-include',
  '@williamcodebox/cordis-plugin-group',
  '@williamcodebox/cordis-plugin-timer',
  '@williamcodebox/cordis-plugin-hmr',
  '@williamcodebox/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@williamcodebox/node-addon-landlock-run',
  '@williamcodebox/node-addon-landlock-run-linux-arm64',
  '@williamcodebox/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@williamcodebox/node-addon-landlock-run': ['src/main.c'],
}
...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@williamcodebox/dsh': ['lib/*.js', 'config'],
  // The Web build emits sourcemaps for browser debugging; publishing them is
  // what the payload policy forbids, so the bundle ships without them.
  '@williamcodebox/omd-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@williamcodebox/omd-base': ['cordis.patch.yml'],
  '@williamcodebox/omd-web-app': ['cordis.patch.yml'],
  '@williamcodebox/omd-headless': ['cordis.patch.yml'],
  '@williamcodebox/omd-tui': ['cordis.patch.yml'],
  '@williamcodebox/omd-client-ui-theme': ['lib/styles'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@williamcodebox/omd-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@williamcodebox/omd-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  '@williamcodebox/omd-skill-badge': ['assets'],
  '@williamcodebox/omd-subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (manifest.name?.startsWith('@williamcodebox/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@williamcodebox/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@williamcodebox/omd-')) {
    const peer = manifest.peerDependencies?.['@williamcodebox/cordis']
