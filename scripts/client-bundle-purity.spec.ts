...
function clientConfigs(id = '@williamcodebox/omd-client-test') {
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@williamcodebox/omd-client-test', ['lib/types/index.js'])
...
describe('client bundle purity gate', () => {
  ...
  it('leaves platform table entries and non-scoped specifiers alone', () => {
    expect(resolveId('@williamcodebox/omd-client-ui-slots')).toBeNull()
    expect(resolveId('@williamcodebox/omd-client-web-react')).toBeNull()
    expect(resolveId('@williamcodebox/omd-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects retired table entries (web-react/store left the 8-entry seed)', () => {
    expect(() => resolveId('@williamcodebox/omd-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@williamcodebox/omd-host-apiproxy/api')).toBeNull()
    expect(resolveId('@williamcodebox/omd-session/surface')).toBeNull()
    expect(resolveId('@williamcodebox/omd-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@williamcodebox/omd-goal/remote')).toBeNull()
    expect(() => resolveId('@williamcodebox/omd-goal')).toThrow(/purity/)
    expect(() => resolveId('@williamcodebox/omd-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@williamcodebox/omd-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @williamcodebox leak', () => {
    expect(() => resolveId('@williamcodebox/omd-agent')).toThrow(/purity/)
    expect(() => resolveId('@williamcodebox/omd-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike (the rewrite arm is gone)', () => {
    expect(() => resolveId('@williamcodebox/omd-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@williamcodebox/omd-client-runtime')).toThrow(/purity/)
    expect(() => resolveId('@williamcodebox/omd-client-ui-layout/client')).toThrow(/purity/)
  })

  it('carries exactly one documented temporary exemption: runtime/client (store engine pending rehoming)', () => {
    expect(resolveId('@williamcodebox/omd-client-runtime/client')).toBeNull()
    const clientChannels = CLIENT_EXTERNALS.filter(
      entry => entry.startsWith('@williamcodebox/') && entry.endsWith('/client'))
    expect(clientChannels).toEqual(['@williamcodebox/omd-client-runtime/client'])
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@williamcodebox/omd-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/@williamcodebox/omd-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@williamcodebox/omd-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@williamcodebox/omd-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/@williamcodebox/omd-client-connection/client.js.map')
