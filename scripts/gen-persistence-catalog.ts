...
const SESSION_PACKAGE = '@williamcodebox/omd-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@williamcodebox/omd-session/types'
 ...
 * top-level declaration (in `@williamcodebox/omd-session`) and any declaration
 * merge inside a `declare module '@williamcodebox/omd-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @williamcodebox/omd-session/known-event-types',
    ' */',
    '',
    '/**',
