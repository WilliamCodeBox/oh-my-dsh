...
packages/    @williamcodebox/omd-<pkg> workspaces at packages/<group>/<pkg>/
  ...
  bundle/      installable omd --profile patch-layer bundles
...
native/      @williamcodebox/node-addon-landlock-run source of record (see native/README.md)
...
pnpm omd --profile headless "task"  # run one task from source (needs DEEPSEEK_API_KEY)
...
- Every npm package is `@williamcodebox/omd-<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@williamcodebox/cordis` is a peerDependency (+ dev) of every harness package.
