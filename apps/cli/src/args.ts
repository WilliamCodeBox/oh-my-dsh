...
const HELP_EXAMPLES = `
Examples:
  omd --profile web                          boot the web profile (same as: omd web)
  omd --profile headless "run the tests"     answer one task, print the result, and exit
  omd --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  omd --profile tui --resume <session>       arguments after the launcher flags reach the app
  omd --profile web --help                   the web app's own flags and help
  omd plugin --profile tui add <package>     install a plugin into the tui profile
`
...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  program
    ...
    .description('omd: boot a DeepSeek Harness profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    ...
    .argument('[args...]', 'arguments for the booted profile\'s app (see: omd --profile <name> --help)')
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: omd web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  if (resolved === undefined) throw new Error('omd: no invocation resolved')
  return resolved
}
