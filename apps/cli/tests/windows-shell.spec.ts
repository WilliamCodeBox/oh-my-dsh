...
import { entryListSchema } from '@williamcodebox/cordis-plugin-include'
import { evaluate } from '@williamcodebox/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@williamcodebox/omd-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@williamcodebox/omd-base', '@williamcodebox/omd-web-app'])
    const profile = loadProfile('omd', 'web', anchor, home)
    ...
    for (const name of ['@williamcodebox/omd-pwsh-sandbox', '@williamcodebox/omd-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@williamcodebox/omd-base'])
    const profile = loadProfile('omd', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
