...
import { boot, loadOverlayPatches } from '@williamcodebox/omd-app-boot'
import SystemPrompt from '@williamcodebox/omd-system-prompt'
import ToolRuntime from '@williamcodebox/omd-tools'
import * as McpClient from '@williamcodebox/omd-mcp-client/src/index.ts'
...
describe('third-party memory MCP example overlays', () => {
  it.each(examples)('parses $file with the documented generic plugin fields', (contract) => {
    ...
    expect(row.name).toBe('@williamcodebox/omd-mcp-client')
