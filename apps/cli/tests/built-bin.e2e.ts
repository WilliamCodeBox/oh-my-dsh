...
import { startMockLlmServer } from '@williamcodebox/omd-llm-mock-server'
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    dsh: { profile: { bundles: ['@williamcodebox/omd-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@williamcodebox/omd-cmdline` and `commander` through the profile module
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  it('defaults to the tui profile and rejects removed subcommands', async () => {
    // A bare `omd` boots the default tui profile; with empty piped stdin the
    // pipe surface quits cleanly instead of demanding --profile.
    const home = mkdtempSync(join(tmpdir(), 'omd-bare-'))
    try {
    ...
    expect(help.stdout).toContain('omd --profile web')
    expect(help.stdout).toContain('omd plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    ...
    try {
      ...
      expect(web.stdout).toContain('Usage: omd --profile web')
      expect(web.stdout).toContain('--port <port>')
      expect(web.stdout).not.toContain('omd web: http://')
      ...
      expect(wildcardHost.stderr).not.toContain('omd web: http://')
      ...
      expect(headlessHelp.stdout).toContain('Usage: omd --profile headless')
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    ...
    try {
      ...
      expect(result.stderr).toContain('omd plugin --profile nope add')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  it('reports a patch-overlay boot failure without hanging', async () => {
    ...
    // refresh drain: omd exited 13 with no diagnostic instead of settling
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    // `omd plugin --profile x add .` from a plugin checkout must install THAT
  ...
  it('activates a dependency that gained dsh.bundle in a later update', async () => {
    ...
    try {
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@williamcodebox/omd-base'])
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@williamcodebox/omd-base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@williamcodebox/omd-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @williamcodebox/omd-base')
      expect(stdout).toContain("name: '@williamcodebox/omd-host-webserver'")
    }, 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@williamcodebox/omd-headless'")
      expect(stdout).not.toMatch(/name: '@williamcodebox\/dsh-host-/)
      expect(stdout).not.toContain("name: '@williamcodebox/omd-web-app'")
      expect(stdout).not.toMatch(/name: '@williamcodebox\/dsh-client-/)
    }, 30_000)
