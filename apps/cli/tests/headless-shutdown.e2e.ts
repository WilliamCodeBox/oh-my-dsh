...
async function runHeadlessPtySmoke(): Promise<string> {
  ...
  try {
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      ...
      private: true,
      dependencies: {},
      dsh: { profile: { bundles: ['@williamcodebox/omd-base', '@williamcodebox/omd-headless'] } },
    }, undefined, 2))
    ...
    if (result.timedOut) {
      throw new Error(`omd headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`omd headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
