// Real-host smoke: spawn `omd web` with a real key, walk the full flow
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`omd web not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /omd web: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`omd web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `omd web` serves every production manifest plugin; until every UI
...
describe('omd web keyless CLI smoke', () => {
