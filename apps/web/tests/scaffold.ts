...
//
// Composition divergences from `omd web`, all deliberate, all via include
...
import Include, { type PatchOptions } from '@williamcodebox/cordis-plugin-include'
import Group from '@williamcodebox/cordis-plugin-group'
import { scrubRequestHeaders, stabilizeFixtureMessageIds } from '@williamcodebox/omd-acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
  healProfilesModuleFallback,
  loadOverlayPatches,
} from '@williamcodebox/omd-app-boot'
import { dshHomePath } from '@williamcodebox/omd-home-paths'
import { settingsNamespace } from '@williamcodebox/omd-settings'
import { LlmAdapter } from '@williamcodebox/omd-llm'
import type {
  LlmModelInfo, LlmProviderInfo, LlmResolvedModelInfo, StreamChunk,
} from '@williamcodebox/omd-llm'
import type { ReplayHandle } from '@williamcodebox/omd-llm-replay'
import { installLlmReplay, parseSessionLog } from '@williamcodebox/omd-llm-replay'
import SessionStore, {
  packChunkRuns,
...
import type {} from '@williamcodebox/omd-host-webserver'
import type {} from '@williamcodebox/omd-agent'
...
// } from '@williamcodebox/omd-client-ui-settings-models'
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const patches: PatchOptions[] = [
    ...
    // (apps/web IS @williamcodebox/omd-web-frontend); only the URL line is silenced.
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@williamcodebox/omd-host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@williamcodebox/omd-client-ui-directory-picker-browse' },
    ] },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@williamcodebox/omd-tool-cordis' },
      ] }]
      : [],
  ...
  try {
    ...
    // `@williamcodebox/cordis-plugin-group` by name.
