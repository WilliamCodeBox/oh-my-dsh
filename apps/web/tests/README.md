...
the deliberate composition divergences from `omd web` — are documented in
...
## Do not import `@williamcodebox/omd-client-*` here
...
`@williamcodebox/omd-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@williamcodebox/omd-client-web` and the boot-manifest
type from `@williamcodebox/omd-client-modules/client`: booting the real shell is what
...
`@williamcodebox/omd-client-runtime/client` because `client/runtime` is reachable
