...
def test_default_config_is_shipped_with_the_package() -> None:
    ...
    assert "@williamcodebox/omd-agent-spine-demo" in config
    assert "@williamcodebox/omd-session-persistence-jsonl" in config
    assert "@williamcodebox/omd-session-checkpoint-policy" in config
